...
KBUILD_CFLAGS   := -Wstrict-prototypes -Wno-trigraphs -fno-strict-aliasing \
		   -fno-common -Werror-implicit-function-declaration \
		   -Wno-format-security -std=gnu89 $(CLANG_FLAGS) \
...
tools/: FORCE
	...
	$(Q)$(MAKE) LDFLAGS= MAKEFLAGS="$(filter --j% -j,$(MAKEFLAGS))" O=$(shell cd $(O) && /bin/pwd) subdir=tools -C $(src)/tools/
